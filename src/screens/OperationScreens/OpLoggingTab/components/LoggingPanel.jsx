...
export default function LoggingPanel ({
  ...
  useEffect(() => { // Sync VFO changes to new QSO (for radio control frequency tracking)
    if (qso?._isNew && vfo?.freq && qso?.freq !== vfo.freq) {
      updateQSO({
        freq: vfo.freq,
        band: bandForFrequency(vfo.freq),
        mode: vfo.mode ?? qso.mode
      })
    }
  }, [vfo?.freq, vfo?.band, vfo?.mode, qso?._isNew, qso?.freq, qso?.mode, updateQSO])
